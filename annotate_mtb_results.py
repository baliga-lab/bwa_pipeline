...
if __name__ == '__main__':
    ...
    parser.add_argument('vcffile', help="original VCF file (SNP/INDEL)")
    parser.add_argument('dbpath', help="path for database files")
    args = parser.parse_args()

    ##### BUILD SPECIAL MAP
    special_map = {}
    reader = vcfpy.Reader.from_path(args.vcffile)
    for record in reader:
        ...
        """
        special_map[pos] = {
            "cov": record.INFO.get("ADP"),
            "cons": "%s" % call.data.get("GT"),  # genotype consensus
            "gt": call.gt_bases,
